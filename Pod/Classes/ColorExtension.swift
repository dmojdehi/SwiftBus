...
private func parseRGBAString(_ rgba: String) -> (red:CGFloat, green:CGFloat, blue:CGFloat, alpha:CGFloat) {
    ...
    // trim any leading '#'
    let rgbaToParse = rgba.trimmingCharacters(in: CharacterSet(charactersIn: "#"))
    
    let index   = rgbaToParse.index(rgba.startIndex, offsetBy: 0)
    let hex     = String(rgbaToParse[index...])
